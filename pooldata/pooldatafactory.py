...
def create_matchup_results(home_win=0, away_win=0, games=[]):
...
def create_matchup(id, round, home=0, away=0, start='', playoff=None, season=None):
    if not playoff:
        playoff = create_matchup_results()
    if not season:
        season = create_matchup_results()
